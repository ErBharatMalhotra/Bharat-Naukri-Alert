...
import { extractEntry } from "../lib/extract.js";
...
export async function runScrape({ limitPerSource = 40 } = {}) {
  ...
  const report = { started_at: new Date().toISOString(), sources: [], total_added: 0, total_updated: 0 };

  for (const src of sources) {
    const srcReport = { id: src.id, raws: 0, added: 0, updated: 0, errors: [] };
    try {
      ...
      const entries = [];
      for (const raw of raws.slice(0, limitPerSource)) {
        const entry = await extractEntry(raw, src);
