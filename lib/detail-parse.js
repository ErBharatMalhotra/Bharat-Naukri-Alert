...
const AGE_LABEL = /\bage\b|\bbirth\b|\bumr[a]|\bआयु\b/i;
const AGE_JUNK = /resizer|compress|calculator|tool|software|pdf|image|language|selection|stage|notified|shortlist|endurance|test|document|verification|medical|examination|required|applicable|wpm|speed|shorthand|skill/i;
...
const BOILER_RE =
  /skip to content|\bmenu\b(?=\s|$)|home\s+latest job|latest job admit card|admit card results admission|about us terms and conditions|terms and conditions contact us|home\s+latest\s+job\s+updates|latest\s+(?:govt|government)\s+(?:job|vacancy)\s+updates/gi;

const NAV_TAIL_RE =
  /\s+-\s+(?:govt|upsc|ssc|railway|banking|defence|teaching|psu|other)\s+jobs?[\s\S]*$|\b(?:govt|upsc|ssc|railway|banking|defence|teaching|psu|other)\s+jobs?(?:[\s,]+(?:govt|upsc|ssc|railway|banking|defence|teaching|psu|other)\s+jobs?)*\s+more\b[\s\S]*$|\bregister\s+for\s+job\s+alerts[\s\S]*$|\bemployment\s+news\s+search\s+jobs[\s\S]*$|\b(?:admit\s+cards?|results?|answer\s+keys?|syllabus)[\s,]+(?:admit\s+cards?|results?|answer\s+keys?|syllabus|important\s+links?)[\s\S]*$/i;
...
function classifyPair(k, v, out) {
  ...
  } else if (AGE_LABEL.test(k) && !out.ageLimit && !AGE_JUNK.test(k + " " + v) && /\d/.test(v)) {
    out.ageLimit = `${k}: ${v}`.slice(0, 150);
  } else if (out.extras.length < 6 && v.length <= 90 && !/^https?:/i.test(v) && !/sarkari result|www\./i.test(k + v)) {
    out.extras.push({ k: k.slice(0, 40), v: scrubSummary(v.slice(0, 90)) });
  }
}
...
export function parseDetailHtml(html) {
  ...
  const out = { summary: "", dates: [], fee: [], vacancy: [], steps: [], links: [], extras: [] };
  try {
    for (const m of html.matchAll(/<a[^>]*href="(https?:\/\/[^"'\s>]+)"[^>]*>([\s\S]*?)<\/a>/gi)) {
      const text = stripTags(m[2]);
      if (!text || text.length > 60) continue;
      if (!/(apply online|official website|notification|click here|download|view result|check result|view details|view more|official notification|syllabus|result|download notification|recruitment|admit card)/i.test(text)) continue;
      ...
      for (const tr of tb[0].matchAll(/<tr[\s\S]*?<\/tr>/gi)) {
        ...
        if (cells.length >= 3) {
          ...
          if (!out._vacHeader && looksLikeVacancyHeader(cells)) {
            out._vacHeader = cells.map((c) => c.slice(0, 200));
            continue;
          }
          if (out._vacHeader && !out._vacClosed && looksLikeVacancyRow(cells)) {
            out._vacRows = out._vacRows || [];
            if (out._vacRows.length < 12) out._vacRows.push(cells.map((c) => c.slice(0, 200)));
            continue;
          }
          if (out._vacRows && /total/i.test(joined)) {
            out._vacRows.push(cells.map((c) => c.slice(0, 200)));
            out._vacClosed = true;
            continue;
          }
          if (out.extras.length < 8 && cells.every((c) => c.length <= 90) && !/^https?:/i.test(joined)) {
            out.extras.push({ k: cells[0].slice(0, 40), v: scrubSummary(cells.slice(1).join(" — ").slice(0, 90)) });
          }
        }
    ...
    for (const ol of html.matchAll(/<(?:ol|ul)[\s\S]*?<\/(?:ol|ul)>/gi)) {
      ...
      if (lis.length >= 2) {
        out.steps = lis.slice(0, 8).map((s) => scrubSummary(s));
        break;
      }
    }
    if (!out.steps.length) {
      const howTo = /how to apply[\s\S]{0,80}?([\s\S]{60,800}?)(?:important|apply now|official|notification|click|$)/i.exec(stripTags(html));
      const seg = howTo?.[1] || "";
      const sents = seg.split(/(?<=[.!?])\s+|\n+/).map((s) => s.trim()).filter((s) => s.length >= 15 && s.length <= 220);
      if (sents.length >= 1) out.steps = sents.slice(0, 8).map((s) => scrubSummary(s));
    }
  ...
  return out.dates.length || out.fee.length || out.vacancy.length || out.steps.length || (out.summary && out.summary.length > 20) ? out : null;
}
