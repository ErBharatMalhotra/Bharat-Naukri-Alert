...
const PORTAL_JUNK_RE =
  /\bwww\.?\s*[.,]?\s*(?:com|in|net|org)\b\.?|\(\s*(?:com|in|net|org)\s*\)|\badvertisements?\b|\badvertise\s+(?:on|here|with us)\b|\bcontact us\b|\bwelcome to\b|\bskip to main content\b|\bsearch for\b|\bmain menu\b/gi;
...
const BRAND_PASSES = [
  /www\.(?:sarkariresult|rojgarresult|freejobalert|sarkarijobfind|sarkariujala)\.(?:com|in|net|org)/gi,
  /(?:sarkariresult|rojgarresult|freejobalert|sarkarijobfind|sarkariujala)\.(?:com|in|net|org)/gi,
...
export function scrubTitle(t = "") {
  ...
  for (const re of BRAND_PASSES) s = s.replace(re, " ");
...
export function scrubSummary(t = "") {
  let s = String(t);
  for (const re of [...BRAND_PASSES, BOILER_RE]) s = s.replace(re, " ");
  s = s.replace(NAV_TAIL_RE, "");
  return s
...
function sweepText(text, out) {
  ...
  if (feeWin) {
    let fm;
    const catRe =
      /([A-Z][A-Za-z]{2,12}(?:\s*\/\s*[A-Za-z]{2,8}){0,3})\s*:?\s*(?:(?:rs\.?|₹|inr)\s*(\d[\d,]{0,7})(?:\s*\/\s*-)?|(\d[\d,]{0,7})\s*\/\s*-)/gi;
