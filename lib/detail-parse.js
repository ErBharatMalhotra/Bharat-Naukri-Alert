...
const PORTAL_BRANDS =
  /\b(more details visit|visit\s+(?:for\s+)?(?:more\s+details)?)?\b(sarkari\s*result[®©]?|rojgar\s*result[®©]?|rojgarresult\.com|free\s*job\s*alert|sarkari\s*job\s*find|sarkariujala|sarkari\s*ujala|sarkariexam)\b[.:,\s]*/gi;
...
export function scrubSummary(t = "") {
  return String(t)
    .replace(BOILER_RE, " ")
    .replace(PORTAL_BRANDS, " ")
    .replace(/\b(?:more details|visit)\s+(?:visit\s+)?(?:rojgar|sarkari|free)[\w\s.]*/gi, " ")
    .replace(/\s{2,}/g, " ")
      ...
      for (const tr of tb[0].matchAll(/<tr[\s\S]*?<\/tr>/gi)) {
        ...
        if (cells.length >= 3) {
          ...
          if (!out._vacHeader && looksLikeVacancyHeader(cells)) {
            out._vacHeader = cells.map((c) => c.slice(0, 70));
            continue;
          }
          if (out._vacHeader && !out._vacClosed && looksLikeVacancyRow(cells)) {
            out._vacRows = out._vacRows || [];
            if (out._vacRows.length < 12) out._vacRows.push(cells.map((c) => c.slice(0, 70)));
            continue;
          }
          if (out._vacRows && /total/i.test(joined)) {
            out._vacRows.push(cells.map((c) => c.slice(0, 70)));
            out._vacClosed = true;
            continue;
          }
