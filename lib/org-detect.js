...
export function postsFromDetails(details) {
  ...
  const header = (details.vacancy[0] || []).join(" ").toLowerCase();
  const postCol = /total|post|vacanc|seat/i.test(header)
    ? (() => { for (let i = 0; i < (details.vacancy[0] || []).length; i++) { if (/total|post|vacanc|seat/i.test(details.vacancy[0][i])) return i; } return -1; })()
    : -1;
  let max = 0;
  for (let r = 1; r < details.vacancy.length; r++) {
    const row = details.vacancy[r];
    const idx = postCol >= 0 ? postCol : (row.length > 1 ? 1 : 0);
