...
import { scrubTitle, scrubSummary } from "./detail-parse.js";
...
const CATEGORY_RULES = [
  ...
  ["job", /\bvacanc(y|ies)\b|\brecruitment\b|\bbharti\b|\bsarkari naukri\b|\bposts?\b.*\bapply\b|\bonline form\b|\bapply online\b/i],
  ["scheme", /\byojana\b|\bscheme\b|\bpm[- ]?\w+\b|\bmission\b/i],
  ["admit-card", /\badmit card\b|\bhall ticket\b/i],
  ["answer-key", /\banswer key\b|\bresponse sheet\b|\bfinal answer\b|\bmodel answer\b|\bmarksheet\b/i],
...
const APPLY_RANGE =
  /apply(?: online)?[^.\n]{0,80}?from\s+(\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})\s*(?:to|-|–)\s*(\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})/i;

export function extractDeadlineRange(text) {
  const m = String(text).match(APPLY_RANGE);
  return m ? parseDateFlexible(m[2]) : null;
}
...
export function heuristicEntry(raw, sourceMeta) {
  const cleanTitle = scrubTitle(raw?.title || "");
  if (!cleanTitle || cleanTitle.length < 12) return null;
  ...
  const deadline = raw.deadline_hint
    ...
    : extractDeadlineText(text) || extractDeadlineRange(text);
  const entry = {
    id: "",
    content_hash: "",
    title: cleanTitle,
    ...
    history: [],
    summary: scrubSummary((raw.description || "").slice(0, 280)),
...
"total_vacancies": number|null (total number of posts/vacancies),
"fee_general": string|null (application fee for General category, e.g. "500/-"),
"fee_sc_st": string|null (fee for SC/ST, e.g. "250/-" or "Nil"),
"pay_scale": string|null (salary/pay scale if mentioned, e.g. "Level-6: Rs 35400-112400")}
Rules: never invent data. If unsure use null. Extract vacancy count from text like "Total 1200 Posts" or table with post names and numbers. Output ONLY JSON.`;

export async function extractEntry(raw, sourceMeta, { useLlm = true, detailHtml = null } = {}) {
  const fallback = heuristicEntry(raw, sourceMeta);
  if (!useLlm || !providerStatus().any) return fallback;
  try {
    let detailText = "";
    if (detailHtml && detailHtml.length > 200) {
      detailText = String(detailHtml)
    ...
    const prompt = `SOURCE ORG: ${sourceMeta.name}\nURL: ${raw.link}\n\nRAW TEXT:\n${`${raw.title}\n${raw.description || ""}`.slice(0, 4000)}${detailText ? `\n\nDETAIL PAGE TEXT:\n${detailText}` : ""}`;
    ...
    const entry = {
      id: "",
      content_hash: "",
      title: scrubTitle(String(parsed.title).slice(0, 300)),
      ...
      last_seen: new Date().toISOString(),
      history: [],
      summary: scrubSummary(String(parsed.summary || "").slice(0, 300)),
      extraction: `llm:${res.provider}`,
    };
    if (parsed.total_vacancies || parsed.fee_general || parsed.pay_scale) {
      entry._llmStructured = {};
      if (parsed.total_vacancies) entry._llmStructured.total_vacancies = parsed.total_vacancies;
      if (parsed.fee_general) entry._llmStructured.fee_general = parsed.fee_general;
      if (parsed.fee_sc_st) entry._llmStructured.fee_sc_st = parsed.fee_sc_st;
      if (parsed.pay_scale) entry._llmStructured.pay_scale = parsed.pay_scale;
    }
    entry.content_hash = contentHash(entry);
    entry.id = entry.content_hash;
    return entry;
